CREATE DATABASE espressoStories WITH ENCODING 'UTF8';
...
CREATE TABLE featuredproduct_branches (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE orders (
    ...
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
    branch_id INTEGER REFERENCES branches(id) ON DELETE CASCADE,
    user_addresses_id  INTEGER REFERENCES user_addresses(id) ON DELETE CASCADE,
